...
declare global {
	function onPlayVideo(index: number): void
	function onEndedVideo(index: number): void
}

let videoIndex = 0
export let playVideoIndex = 0
...
export async function clearQueue() {
	...
	const lenght = finishVideoIndexQueue.length
	for (let i = 0; i < lenght; i++) {

		//閉じるページをアクティブ状態にする
		const page = videoPages[finishVideoIndexQueue[0]]
